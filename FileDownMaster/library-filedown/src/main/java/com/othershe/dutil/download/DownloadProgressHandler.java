            ...
            switch (mCurrentState) {
                case START:
                    ...
                    if (downloadCallback != null) {
                        downloadCallback.onStart(currentLength, totalLength, com.othershe.dutil.utils.Utils.getPercentage(currentLength, totalLength));
                    }
                    break;
                case PROGRESS:
                    synchronized (this) {
                        ...
                        downloadData.setPercentage(com.othershe.dutil.utils.Utils.getPercentage(currentLength, totalLength));

                        if (downloadCallback != null && (System.currentTimeMillis() - lastProgressTime >= 20 || currentLength == totalLength)) {
                            downloadCallback.onProgress(currentLength, totalLength, com.othershe.dutil.utils.Utils.getPercentage(currentLength, totalLength));
                    ...
                    synchronized (this) {
                        ...
                        if (tempChildTaskCount == childTaskCount || mLastSate == PAUSE || mLastSate == ERROR) {
                            ...
                            if (isSupportRange) {
                                ...
                                com.othershe.dutil.utils.Utils.deleteFile(new File(path, name + ".temp"));
                            }
                            com.othershe.dutil.utils.Utils.deleteFile(new File(path, name));
                            if (downloadCallback != null) {
                ...
                case PAUSE:
                    synchronized (this) {
                        if (isSupportRange) {
                            Db.getInstance(context).updateProgress(currentLength, com.othershe.dutil.utils.Utils.getPercentage(currentLength, totalLength), PAUSE, url);
                        }
                        tempChildTaskCount++;
                ...
                case DESTROY:
                    synchronized (this) {
                        if (isSupportRange) {
                            Db.getInstance(context).updateProgress(currentLength, com.othershe.dutil.utils.Utils.getPercentage(currentLength, totalLength), DESTROY, url);
                        }
                    }
                    break;
                case ERROR:
                    if (isSupportRange) {
                        Db.getInstance(context).updateProgress(currentLength, com.othershe.dutil.utils.Utils.getPercentage(currentLength, totalLength), ERROR, url);
                    }
                    if (downloadCallback != null) {
